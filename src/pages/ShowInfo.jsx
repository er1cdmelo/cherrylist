...
import { WatchListData, FavoritesData } from "../components/providers/watchlist";
...
const ShowInfo = () => {
  ...
  if(watchlist !== null) {watchlist = JSON.parse(localStorage.getItem('watchlist'))}
  let favorites = localStorage.getItem('favorites')
  if(favorites !== null) {favorites = JSON.parse(localStorage.getItem('favorites'))}
  ...
  useEffect(() => {
    if (showImages.length) {
      ...
      setBanner('https://media.istockphoto.com/photos/black-canvas-with-delicate-grid-to-use-as-background-or-texture-picture-id1143755923?k=20&m=1143755923&s=612x612&w=0&h=xM-CYFltFp1rq1lxVv0T7zsPlbU1IfVniXDcAXZbaKw=')
    ...
    <div className={styles.global_container}>
      {show ? (
        <div className={styles.card_container}>
          ...
          {showImages && (
            <img className={styles.banner} src={banner} alt="background" />
          )}
        </div>
      ) : (
        <Loading />
